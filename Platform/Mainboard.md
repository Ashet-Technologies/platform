...
| Signal | Semantics |
| --- | --- |
| `I2C_SCL` | System I²C bus clock lane. 3.3 V, open collector. |
| `I2C_SDA` | System I²C bus data lane. 3.3 V, open collector. |
...
The Mainboard-facing `I2C_SCL` and `I2C_SDA` pins are connected directly to the PCA9547 on the Backplane. The PCA9547 uses I²C address `0x77`.
...
A Mainboard **must** react to `/RESET` going low. If that reset handling causes the Mainboard to boot again, it must resample `/FAB_RESET`.
...
To the host, a Mainboard operating in Not-so-mainboard mode is indistinguishable from a regular Expansion Card.
