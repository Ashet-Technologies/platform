...
A uniform resource partition makes every Expansion Slot structurally equivalent and avoids per-Slot configuration tables.

The same Low-Level-Driver firmware can therefore be loaded for different Slots and compute its own resources from `COGID`.
